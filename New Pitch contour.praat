...
# change these as you prefer them (and/or add things from the form menu below)
...
form Pitch contour parameters (Vers. 2.6):
	...
	choice Method: 3
		...
		button at position
	choice Normalizing: 3
		button none
		button z_score
		button subtract mean
	comment Prefix for result file:
	text prefix
	comment Suffix for result file:
	text suffix
	comment ______________________________________________________________________________________________________________
	real left_F0_range 75.0
...
result_file$ = directory$ + prefix$ + "pitch_contour_results"+ suffix$ + ".txt"
#filedelete 'result_file$'
if fileReadable (result_file$)
	pause The result file 'result_file$' already exists! Do you want to overwrite it?
	filedelete 'result_file$'
endif
...
fileappend 'result_file$' 'tab$'	'newline$'
