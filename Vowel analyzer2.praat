...
form Vowel Analyzer
	...
	sentence Results_file _Vowel analyzer2.txt
	optionmenu Use_targets_file 2
		option yes
		option no
	sentence Targets_file Vowel analyzer2.txt
	comment Tiers:
	sentence Vowel_tier Seg
	...
	optionmenu Extract_pitch: 1
		option yes
		option no
	integer left_Pitch_range_(Hz) 75
	integer right_Pitch_range_(Hz) 800
	comment Analyst:
	sentence Initials
endform
...
procedure InitializeResultsFile
	...
	if measurement_points = 1
		...
		header_middle$ = "F1 0%	F1 50%	F1 100%	F2 0%	F2 50%	F2 100%	F3 0%	F3 50%	F3 100%	"
		if extract_pitch = 1
			header_middle$ = "F0 0%	F0 50%	F0 100%	" + header_middle$
		endif
	elsif measurement_points = 3
